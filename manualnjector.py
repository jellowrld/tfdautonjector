...
class DLLInjectorGUI:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Jell's TFD Manual DLL Injector")
    ...
    def build_ui(self):
        ...
        ttk.Label(self.root, text="⚠️ Use responsibly. DLL injection may violate game terms.", foreground="#ff4444").pack(pady=(10, 5))
    ...
    def wait_for_process_stable(self, process_name):
        ...
        for _ in range(10):
            ...
            if proc and psutil.pid_exists(proc.info["pid"]):
                time.sleep(1)  # Wait for process to stabilize
                return proc
        return None
    ...
    def manual_map_dll(self, dll_path, pid):
        try:
            ...
            try:
                ...
                remote_base = self.kernel32.VirtualAllocEx(h_process, None, image_size, VIRTUAL_MEM, PAGE_EXECUTE_READWRITE)
                if not remote_base:
                    self.log(f"Memory allocation failed: {self.get_last_error()}", level="error")
                    return False

                # Write headers
                headers = pe.generate_image()
                ...
                thread_id = c_ulong(0)
                h_thread = self.kernel32.CreateRemoteThread(h_process, None, 0, dllmain_addr, remote_base, 0, byref(thread_id))
                if not h_thread:
                    self.log(f"Failed to call DllMain: {self.get_last_error()}", level="error")
                    return False
    ...
    def apply_relocations(self, pe, h_process, remote_base, delta):
        ...
        reloc_data = pe.get_data(reloc_rva, reloc_size)
        ...
        while offset < reloc_size:
            ...
            for i in range(entry_count):
                ...
                if entry.Type == 10:  # IMAGE_REL_BASED_DIR64
                    ...
                    old_value = c_uint64()
                    self.kernel32.ReadProcessMemory(h_process, reloc_addr, byref(old_value), sizeof(old_value), None)
                    new_value = old_value.value + delta
                    self.kernel32.WriteProcessMemory(h_process, reloc_addr, byref(c_uint64(new_value)), sizeof(c_uint64), None)
    ...
    def resolve_imports(self, pe, h_process, remote_base):
        ...
        for entry in pe.DIRECTORY_ENTRY_IMPORT:
            ...
            if not h_module:
                ...
                if not h_module:
                    self.log(f"Failed to load {dll_name}", level="error")
                    return False

            for imp in entry.imports:
                if imp.name:
                    func_addr = self.kernel32.GetProcAddress(h_module, imp.name)
                    if not func_addr:
                        self.log(f"Failed to resolve {imp.name.decode()} in {dll_name}", level="error")
                        return False
                    self.kernel32.WriteProcessMemory(h_process, remote_base + imp.address, byref(c_uint64(func_addr)), sizeof(c_uint64), None)
        ...
        try:
            ...
            for i in range(module_count):
                ...
                if mod_name.value.lower() == dll_name.lower():
                    ...
                    return
            self.log("Could not locate injected DLL.", level="warning")
        finally:
            self.kernel32.CloseHandle(h_process)
    ...
    def uninject_dll(self):
        ...
        try:
            # For manual mapping, free the allocated memory
